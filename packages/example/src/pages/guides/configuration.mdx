...
Site-wide search is provided by the theme. The only requirement for a page to show up in the results is for it to have `title` set in the [frontmatter](/guides/MDX#frontmatter).
...
To disable the `Edit in Github` link, remove the `repository` option. If the options object is now empty, delete it all together.
